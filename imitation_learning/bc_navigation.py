...
def process_image(client, img_res):
    image_response = client.simGetImages(
        [
            airsimdroneracingvae.ImageRequest(
                "0", airsimdroneracingvae.ImageType.Scene, False, False
            )
        ]
    )[0]
    img_1d = np.fromstring(
        image_response.image_data_uint8, dtype=np.uint8
    )  # get numpy array
    img_bgr = img_1d.reshape(
...
def move_drone(client, vel_cmd):
    ...
    yaw_mode = airsimdroneracingvae.YawMode(
        is_rate=True, yaw_or_rate=vel_cmd[3] * 180.0 / np.pi
    )
    client.moveByVelocityAsync(
        vel_cmd[0], vel_cmd[1], vel_cmd[2], duration=0.1, yaw_mode=yaw_mode
    )
...
if __name__ == "__main__":
    ...
    time.sleep(0.01)
    client.setTrajectoryTrackerGains(
        airsimdroneracingvae.TrajectoryTrackerGains().to_list(), vehicle_name=drone_name
    )
    time.sleep(0.01)
    ...
    offset = [0, 0, -0]
    gate_poses = racing_utils.trajectory_utils.RedGateSpawnerCircle(
        client,
        num_gates=8,
        radius=8,
        radius_noise=gate_noise,
        height_range=[0, -gate_noise],
        track_offset=offset,
    )
    ...
    while True:
        ...
        if count == max_count:
            ...
            print(
                "Avg network time over {} iterations: {} ms | {} Hz".format(
            ...
            avg_freq = 1.0 / avg_time
            print(
                "Avg loop time over {} iterations: {} ms | {} Hz".format(
                    max_count, avg_time * 1000, avg_freq
                )
            )
