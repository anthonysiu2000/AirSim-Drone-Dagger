...
class VelRegressor():
    def __init__(self, regressor_type, bc_weights_path, feature_weights_path=None, latent_space_constraints=True, capsule_network=False):
        ...
        if self.regressor_type == 'full':
            ...
            if latent_space_constraints is True:
                self.cmvae_model = racing_models.cmvae.CmvaeDirect(n_z=10, gate_dim=4, res=64, trainable_model=False, capsule_network=capsule_network)
            else:
                self.cmvae_model = racing_models.cmvae.Cmvae(n_z=10, gate_dim=4, res=64, trainable_model=False, capsule_network=capsule_network)
